...
def graph_create(filename_results: str, filename_games: str) -> None:
    ...
    teams = results.keys()
    ranks = [value * 400 if value != 0.0 else 1200 for value in results.values()]
    ...
    nx.draw(G, pos=nx.circular_layout(G), arrows = True, arrowsize = 15, node_color=colors_to_choose, node_size=ranks, with_labels=True, labels=node_labels, font_family="Times New Roman", font_color='black', font_size = 11, font_weight = 'bold')
